...
const getArtWorks = async (req,res,next) => {
    const art = await pool.query("SELECT sub_category.sub_category_name,sub_category.quantity_stock,category.category_name,product.product_title,product.price,product_description,artist.stage_name FROM sub_category LEFT JOIN category ON WHERE sub_category.category_id = category.category_id LEFT JOIN ON WHERE category.category_id = product.category_id LEFT JOIN ON WHERE product.artist_id = artist.artist_id")
    if(!art.rows.length) return next(new NotFoundError("Art work not available"))
    res.status(200).json({data:art.rows[0]})
}

const getSpecificArtWork = async (req,res,next) => {
    const {artworkId} = req.params
    const art = await pool.query("SELECT sub_category.sub_category_name,product.product_title,product.price,product_description,dimension.dimension,dimension.unit FROM product LEFT JOIN sub_category ON WHERE product.product_id = sub_category.product_id LEFT JOIN ON WHERE sub_category.sub_category_id = dimension.sub_category_id WHERE product.product_id = $1",[artworkId]);
    if(!art.rows.length) return next(new NotFoundError("Art work not available"))
    res.status(200).json({data:art.rows[0]})
}
...
const getCollection = async(req,res,next) => {
    ...
    res.status(200).json({data:collection.rows[0]})
}
...
const getSubCollection = async(req,res,next) => {
    ...
    res.status(200).json({data:subcol.rows[0]})
}
